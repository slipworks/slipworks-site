exports.handler = async function(event) {
  ...
  const html = '<!doctype html><meta charset="utf-8"><title>OAuth Callback</title><body>Completing sign-in... You can close this window.</body><script>(function(){try{var p=new URLSearchParams(location.search);var msg={type:"oauth:done",provider:"github",success:!p.get("error"),code:(p.get("code")?p.get("code"):null),access_token:(p.get("access_token")?p.get("access_token"):null),state:(p.get("state")?p.get("state"):null),error:(p.get("error")?p.get("error"):null)};if(window.opener){if(typeof window.opener.postMessage==="function"){window.opener.postMessage(msg,"https://splendid-hummingbird-b1b121.netlify.app");}}setTimeout(function(){try{window.close();}catch(e){}},120);}catch(_){}})();</script>';
