...
exports.handler = async function(event){
  ...
  function pageFromB64(b64){return '<!doctype html><meta charset="utf-8"><title>OAuth Callback</title><body>Completing sign-in...</body><script>(function(){try{var ORIGIN="' + ORIGIN + '";var MSG=atob("' + "' + b64 + '" + '");if(window.opener&&typeof window.opener.postMessage==="function"){window.opener.postMessage(MSG,ORIGIN);}setTimeout(function(){try{if(window.opener&&!window.opener.closed){window.close();}}catch(_){}} ,120);}catch(_){}})();</script>'; }
  function errPage(message){ var s='authorization:github:error:'+JSON.stringify({message:String(message
  ...
  try{
    ...
    const s = 'authorization:github:success:'+JSON.stringify({token:token});
    const b64 = Buffer.from(s).toString('base64');
    const html = pageFromB64(b64);
